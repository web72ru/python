import requests, lxml.html, json, csv
from al_re import reg

session = requests.Session()

исходная_ссылка = ['https://api.petrovich.ru/catalog/v2.3/products/105679?city_code=rf&client_id=pet_site']
шапка = {
...
страница = получаем_хтмл(исходная_ссылка[0])
содержимое_страницы = страница.text
словарь = json.loads(содержимое_страницы)

ключи = list(словарь['data']['product'])
значения = list(словарь['data']['product'].values())

характеристики = словарь['data']['product']['properties']

ключи_характеристик_список = []
значения_характеристик_список = []

for ключ in характеристики:
    ключи_характеристик_список.append(ключ['title'])

for значение in характеристики:
    значения_характеристик_список.append(значение['value'][0]['title'])

изображения = словарь['data']['product']['images']
ключи_изображений = ['иза' for иза in range(1, len(изображения))]

категории = словарь['data']['product']['breadcrumbs']
ключи_категории = ['категория' for категория in range(0, len(категории))]
значения_категории = [категории[категория]['title'] for категория in range(0, len(категории))]

ключ_название = ['название']
значение_название = [словарь['data']['product']['title']]

ключ_описание = ['описание']
значение_описание = [словарь['data']['product']['description']]


def запись(путь):
    with open(путь, 'w', newline='') as файл:
        записчик = csv.writer(файл, delimiter=';')
        записчик.writerow(ключ_название + ключи_категории + ключ_описание + ключи_характеристик_список + ключи_изображений)
        записчик.writerow(значение_название + значения_категории + значение_описание + значения_характеристик_список + изображения)
